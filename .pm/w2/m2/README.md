...
**Worker:** worker1 **Goal:** Turn the six real-host failures on Muse 1.2.1 into either fixed behavior with evidence or recorded host defects with reproductions, so the README's supported-host statement matches what the adapter actually does on the host the current Muse launcher installs. **Status:** todo
...
failed identically at `852a612` with [m1](../done/m1/README.md) stashed, so none of them is caused
...
The adapter starts sessions with `approvalMode: "onRequest"` and never names a profile, so where
`:auto-review` is attached, and whether an adapter-visible public field can avoid or clear it, is
the open question. The README currently attributes this to legacy exec continuation; the
`muse-sdk-live` failure shows it also affects an SDK-created session, so that attribution needs
...
| ---- | ----------------------------------------------------------------------- | --- | ---------------------- |
| t001 | Reproduce and classify the six failures against public host surfaces    | 45m | —                      |
| t002 | Resolve or record the `:auto-review` session-composition failure        | 60m | w2/m2/t001             |
| t003 | Resolve or record HTTP MCP endpoint diagnostics on 1.2.1                | 60m | w2/m2/t001             |
| t004 | Adoption surface: correct the supported-host statement and attributions | 30m | w2/m2/t002, w2/m2/t003 |
| t005 | Simplify milestone changes                                              | 30m | w2/m2/t004             |
| t006 | CI and behavior coverage                                                | 45m | w2/m2/t004, w2/m2/t005 |
| t007 | Close out the milestone                                                 | 15m | w2/m2/t006             |
...
- **Source:** real-host measurement while verifying [w2/m1](../done/m1/README.md) on 2026-09-14, and
