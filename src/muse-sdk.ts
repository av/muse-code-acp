...
export function spawnMuseSdkTurn(options: MuseSdkOptions): MuseSdkHandle {
  ...
  const done = (async (): Promise<PromptResponse> => {
    try {
      ...
      const decideApproval = async (view: MuseApprovalRequest): Promise<void> => {
        ...
        try {
          ...
          if (
            ...
            if (!fresh) {
              options.logger.log(
                `muse-sdk: approval ${view.approvalId} requirement ${view.currentRequirementId.sourceIndex} settled despite internal error; continuing`,
              );
              return;
            }
            if (
              ...
              options.logger.log(
                `muse-sdk: approval ${view.approvalId} requirement ${view.currentRequirementId.sourceIndex} changed during internal error; re-asking`,
              );
              return;
            }
            try {
              ...
              if (
                retryError instanceof MspError &&
                ...
                options.logger.log(
                  `muse-sdk: approval ${view.approvalId} requirement ${view.currentRequirementId.sourceIndex} was superseded on retry`,
                );
                return;
              }
              if (cancelled || options.isCancelled?.()) {
              ...
              options.logger.log(
                `muse-sdk: approval ${view.approvalId} decision retry rejected (MSP ${retryError instanceof MspError ? `${retryError.code} ${retryError.kind}` : String(retryError)})`,
              );
              failTurn(
                retryError instanceof MspError
      ...
      const askOneUserInput = async (
        request: MuseUserInputRequest & { userInputId: string; turnId: string },
      ): Promise<void> => {
        if (!connection) {
        ...
        try {
          ...
          if (isElicitationUnsupported(error)) {
            ...
            options.logger.log(
              "muse-sdk: elicitation unsupported; stopping the turn and asking in chat",
            );
            try {
            ...
            try {
              await connection.command(
                "turn/cancel",
                { sessionId: options.sessionId, turnId: request.turnId },
                { maxAttempts: 1 },
              );
            } catch (cancelError) {
              if (!cancelled && !options.isCancelled?.()) {
      ...
      const watchdog = new PendingWorkWatchdog(
        stallLimitMs(options.env),
        Date.now,
      ...
      const toolBusy = (): boolean => {
        if (!turnId || !session.fold.current) return false;
        return session.fold.items.list().some(
          (item) =>
            item.turnId === turnId &&
      ...
      const clientDeciding = (): boolean => {
        if (decidingApprovals.size > 0) return true;
        if (!session.fold.current || !turnId) return false;
        for (const pendingInput of session.fold.pendingUserInputs()) {
          const request = pendingInput as unknown as { userInputId: string; turnId: string };
          if (request.turnId === turnId && userInputs.has(request.userInputId)) return true;
        }
        return false;
      };
      ...
      const checkPendingWork = (): void => {
        ...
        if (clientDeciding()) {
          ...
          if (hungTool) {
            failTurn(
              new Error(
                `${hungTool}. A tool was still running and published nothing further; the turn was stopped instead of hanging.`,
              ),
            );
            if (connection && turnId) {
              void connection
                .command(
                  "turn/cancel",
                  { sessionId: options.sessionId, turnId },
                  { maxAttempts: 1 },
                )
                .catch(() => {});
            }
          ...
          if (silent) {
            failTurn(
              new Error(
                `${silent}. The Muse host stopped emitting progress; the turn was stopped instead of hanging.`,
              ),
            );
            if (connection && turnId) {
              void connection
      ...
      const watchTurn = (active: typeof turn) => {
        const items = (async () => {
      ...
      let continuations = 0;
      let response: Awaited<ReturnType<typeof sdkTerminalResponse>>;
      for (;;) {
        ...
        const usage = session.fold.sessionState.get("session/tokenUsage") as
          | { finishReason?: string; turnId?: string }
          | undefined;
        ...
        if (!unfinished || continuations >= MAX_EARLY_CONTINUATIONS) {
          if (unfinished) response = { stopReason: "max_turn_requests" };
          break;
        }
        continuations++;
