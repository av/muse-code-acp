...
export function mcpStartupFailure(error: unknown): string | undefined {
  const text = String(error);
  if (!text.includes("Required MCP server") || !text.includes("failed during startup")) return;
  if (text.includes("authentication failed")) return "authentication rejected";
  if (text.includes("violated the MCP protocol")) return "invalid MCP response";
  if (text.includes("could not be reached")) return "server unreachable or process unavailable";
