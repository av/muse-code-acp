...
import { OUTPUT_EXTENSION, OUTPUT_METHOD, type OutputPage } from "../stored-output.js";
...
for (const negotiated of [false, true])
  it(`reads exact stored bytes across restart without provider replay (negotiated=${negotiated})`, async () => {
    ...
    let restored: ReturnType<typeof connectTestClient> | undefined;
    ...
    try {
      ...
      const refs = () =>
        client.updates.flatMap((n) =>
          n.update._meta?.[OUTPUT_EXTENSION]
            ? [
                n.update._meta[OUTPUT_EXTENSION] as {
                  itemId: string;
                  outputRef: string;
                  byteLen: number;
                },
      ...
      const cards = client.updates.filter((n) =>
        ["tool_call", "tool_call_update"].includes(n.update.sessionUpdate),
      );
      ...
      if (negotiated && supported) expect(refs().length).toBeGreaterThan(0);
      if (!negotiated || !refs().length) {
        expect(refs()).toHaveLength(0);
        await expect(
          c.request(OUTPUT_METHOD, { sessionId, itemId: "missing", outputRef: "missing" }),
      ...
      const page = (await c.request(OUTPUT_METHOD, params)) as OutputPage;
      ...
      expect(Buffer.from(tail.content, "base64")).toEqual(
        Buffer.from("output-marker\n".repeat(30000)).subarray(299990, 300000),
      );
      for (const wrong of [
      ...
      await expect(
        c.request(OUTPUT_METHOD, { ...params, sessionId: other.sessionId }),
      ).rejects.toMatchObject({ code: -32600 });
      expect(p.requests()).toHaveLength(before);
      await c.request(methods.agent.session.prompt, {
      ...
      expect(
        restored.updates.some(
          (n) =>
            (n.update._meta?.[OUTPUT_EXTENSION] as { outputRef?: string })?.outputRef ===
            ref.outputRef,
        ),
      ).toBe(true);
