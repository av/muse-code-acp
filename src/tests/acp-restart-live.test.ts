...
describe("ACP process restart continuity (real Muse host)", () => {
  ...
  it.each(["load", "resume"] as const)(
    "restarts the ACP process, binds via %s, and continues the same session",
    async (method) => {
      ...
      try {
        ...
        try {
          ...
          await expect(
            agent1.ctx.request(methods.agent.session.close, { sessionId }),
          ).resolves.toEqual({});
        } finally {
        ...
        try {
          ...
          if (method === "load")
            ...
            expect(
              agent2.updates.some((n) =>
                [
                  "user_message_chunk",
                  "agent_message_chunk",
                  "tool_call",
                  "tool_call_update",
                ].includes(n.update.sessionUpdate),
              ),
            ).toBe(false);
          await expect(
