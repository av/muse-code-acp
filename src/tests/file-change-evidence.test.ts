...
function track(cwd: string, name: string, text: string) {
  const path = join(cwd, name);
  writeFileSync(path, text);
  execFileSync("git", ["add", "--", name], { cwd });
  return path;
}
function item(path: string, content: string, status = "completed"): FoldedItem {
  return {
    itemId: "item",
    callId: "call",
    turnId: "turn",
...
it("shows actual dirty working-tree preimages, updates repeated observations, and excludes unrelated dirty files", () => {
  ...
  expect(evidence.present(item(path, "agent text"))).toContainEqual(
  ...
  expect(evidence.present(item(path, "later text"))).toContainEqual(
    expect.objectContaining({ type: "diff", oldText: "agent text", newText: "later text" }),
  );
...
it("distinguishes known absence from an unknown preimage and a later deletion", () => {
  ...
  expect(evidence.present(item(path, "created"))).toContainEqual(
    expect.objectContaining({ type: "diff", oldText: null, newText: "created" }),
  );
  const unknown = join(cwd, "untracked");
  writeFileSync(unknown, "existing content");
  expect(evidence.present(item(unknown, "existing content"))?.some((c) => c.type === "diff")).toBe(
    false,
  );
  unlinkSync(path);
...
it("never turns failed tools or concurrent post-write edits into precise diffs", () => {
  ...
  expect(
    evidence.present(item(path, "unexpected user text", "failed"))?.some((c) => c.type === "diff"),
  ).toBe(false);
});
it("can refresh a bounded preimage immediately before a host-offered approval", () => {
  ...
  expect(evidence.present(item(path, "allowed"))).toContainEqual(
    expect.objectContaining({
      type: "diff",
...
it.each(["large", "binary", "invalid-utf8", "symlink"])("keeps %s evidence unavailable", (kind) => {
  const cwd = workspace();
  const path = track(cwd, "target", "old");
  if (kind === "large") writeFileSync(path, "x".repeat(65537));
  if (kind === "binary") writeFileSync(path, "x\0y");
  if (kind === "invalid-utf8") writeFileSync(path, Buffer.from([0xff]));
  if (kind === "symlink") {
    unlinkSync(path);
    symlinkSync(join(cwd, "other"), path);
  }
  const evidence = new FileChangeEvidence(cwd);
  if (kind === "symlink") unlinkSync(path);
...
it("bounds report size, deduplicates paths and marks cancellation unavailable", () => {
  const cwd = workspace();
  const evidence = new FileChangeEvidence(cwd);
  for (let i = 0; i < 1030; i++) evidence.present(item(join(cwd, `file-${i}`), ""));
...
it("does not attribute shell moves or child changes without native file declarations", () => {
  ...
  expect(evidence.present({ ...item(path, ""), kind: "worker" } as FoldedItem)).toBeUndefined();
...
it("keeps snapshots within the aggregate content budget", () => {
  const cwd = workspace();
  for (let i = 0; i < 17; i++) track(cwd, `file-${String(i).padStart(2, "0")}`, "x".repeat(65536));
  const evidence = new FileChangeEvidence(cwd);
