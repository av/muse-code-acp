...
it("delivers attributed embedded bytes, preserves user titles across restart, and dispatches local commands", async () => {
  ...
  let second: ReturnType<typeof connectTestClient> | undefined;
  try {
    ...
    const local = (text: string) =>
      c.request(methods.agent.session.prompt, { sessionId, prompt: [{ type: "text", text }] });
    ...
    expect(
      second.updates.some(
        (n) =>
          ...
          n.update.title === "search: needle-marker",
      ),
    ).toBe(true);
    expect(
      second.updates
...
it("injects compatible steering into a real active turn and rejects idle fallback", async () => {
  ...
  try {
    ...
    await expect
      .poll(
        () => p.requests().some((r) => JSON.stringify(r.input).includes("compat-steer-start")),
        { timeout: 10000 },
      )
      .toBe(true);
    await expect
      .poll(
        () => client.updates.some((n) => typeof n.update._meta?.["muse/activeTurnId"] === "string"),
        { timeout: 10000 },
      )
      .toBe(true);
    expect(
      await c.request("_session/steering", {
        sessionId,
    ...
    const before = p.requests().length;
    await expect(
      c.request("_session/steering", { sessionId, prompt: [{ type: "text", text: "idle" }] }),
