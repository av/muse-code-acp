...
async function setup(backend: "sdk" | "exec" = "sdk") {
...
describe("mode selection through ACP config options", () => {
  it.each(["sdk", "exec"] as const)(
    "%s advertises the same modes through both interfaces",
    async (backend) => {
  ...
  it("uses the existing persistent mode selection and keeps both client surfaces synchronized", async () => {
    ...
    expect(
      client.updates.some(
        (n) =>
          n.update.sessionUpdate === "current_mode_update" && n.update.currentModeId === "readOnly",
      ),
    ).toBe(true);
    await ctx.request(methods.agent.session.setMode, { sessionId, modeId: "default" });
    const updates = client.updates.flatMap((n) =>
      n.update.sessionUpdate === "config_option_update" ? [n.update.configOptions] : [],
    );
    ...
    expect(
      client.updates.some(
        (n) =>
          ...
          n.update.configOptions.some((o) => o.id === "mode" && o.currentValue === "plan"),
      ),
    ).toBe(true);
