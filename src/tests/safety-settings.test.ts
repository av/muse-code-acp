...
it("validates stored selections and requires explicit sandbox opt-in", () => {
  ...
  expect(() => selectSafety(undefined, "sandbox", "disabled", normal)).toThrow(
    /MUSE_CODE_ACP_ALLOW_YOLO/,
  );
  expect(selectSafety(undefined, "nativeApprovalPolicy", "allowAll", normal).sandbox).toBe(
    "enabled",
  );
  expect(
    selectSafety(undefined, "sandbox", "disabled", {
      env: { MUSE_CODE_ACP_ALLOW_YOLO: "1" },
      isRoot: false,
    }).nativeApprovalPolicy,
  ).toBe("onRequest");
  expect(() =>
    selectSafety(undefined, "sandboxNetwork", "enabled", { env: {}, isRoot: true }),
  ).toThrow(/root/);
  expect(availableModes({ env: {}, isRoot: true }, "sdk").map((m) => m.id)).not.toContain(
    "bypassApprovals",
  );
});
