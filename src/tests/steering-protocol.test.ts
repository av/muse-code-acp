...
describe("parseSteeringRequest", () => {
  ...
  it("rejects requests missing session or turn ids", () => {
    expect(() =>
      parseSteeringRequest({ expectedTurnId: "turn-1", prompt: [{ type: "text", text: "x" }] }),
    ).toThrow(/steering requires/);
    expect(() =>
      parseSteeringRequest({ sessionId: "s", prompt: [{ type: "text", text: "x" }] }),
    ).toThrow(/steering requires/);
  });

  it("rejects an empty prompt", () => {
    expect(() => parseSteeringRequest({ sessionId: "s", expectedTurnId: "t", prompt: [] })).toThrow(
      /steering requires/,
    );
  });
