...
async function mcpFixture(mode: "ok" | "unauthorized" | "malformed" = "ok") {
  ...
  if (!address || typeof address === "string") throw new Error("MCP fixture did not bind");
  return {
...
describe.skipIf(!available)("real Muse HTTP MCP", () => {
  it.each(["ok", "unauthorized", "malformed", "unreachable"] as const)(
    "preserves truthful diagnostics for %s endpoints",
    async (mode) => {
      const endpoint = await mcpFixture(mode === "unreachable" ? "ok" : mode);
      if (mode === "unreachable") await endpoint.close();
      try {
        const provider = await startLoopbackProvider({
          scriptedToolCallWhen:
            mode === "ok" ? ['"name":"ping"', "m16-user-request"] : ["never-script"],
        ...
        try {
          ...
          if (mode === "ok") await expect(prompt).resolves.toEqual({ stopReason: "end_turn" });
          else await expect(prompt).rejects.toMatchObject({ code: -32603 });
          if (mode === "ok") {
            expect(endpoint.calls.some((call) => call.method === "tools/call")).toBe(true);
            expect(
              provider
                .requests()
                .some((request) => JSON.stringify(request.input).includes("m16-pong")),
            ).toBe(true);
          } else expect(endpoint.calls.some((call) => call.method === "tools/call")).toBe(false);
          if (mode !== "unreachable") {
            expect(endpoint.calls.length).toBeGreaterThan(0);
            expect(
              endpoint.calls.every((call) => call.authorization === "Bearer private-m16-header"),
            ).toBe(true);
          }
          if (mode === "ok") {
            ...
            expect(
              endpoint.calls
                .slice(beforePeer)
                .every((call) => call.authorization === "Bearer private-m16-header-peer"),
            ).toBe(true);
            ...
            expect(
              endpoint.calls
                ...
                .every((call) => call.authorization === "Bearer private-m16-header-new"),
            ).toBe(true);
          ...
          if (mode !== "ok") expect(status).toContain(failure[mode]);
          else expect(status).not.toContain("Last execution observed");
