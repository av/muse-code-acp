...
function storeRaw(sessionId: string, env: Record<string, string | undefined>, raw: string): void {
...
describe("session preferences", () => {
  ...
  it("rejects stored documents with a wrong schema version", () => {
    ...
    expect(() => readSessionPreferences("session-1", env)).toThrow(
      /Invalid stored ACP session preference/,
    );
  });
