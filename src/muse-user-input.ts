 ...
 */
export function isElicitationUnsupported(error: unknown): boolean {
...
function containsMethodNotFound(value: unknown, depth = 0): boolean {
  if (value == null || depth > 3) return false;
  if (typeof value === "string") return /method not found/i.test(value);
 ...
 */
export function userInputToChatMessage(request: MuseUserInputRequest): string {
  const lines: string[] = [];
  for (const question of request.questions) {
  ...
  lines.push(
    "_I stopped here so I do not pick for you. Reply in chat with your choice and I will continue on the next turn._",
  );
  return lines.join("\n");
}
...
export async function cancelUserInput(
  connection: Connection,
  sessionId: string,
  request: MuseUserInputRequest,
  reason: string,
  ...
  await connection.command(
    "userInput/cancel",
    { sessionId, userInputId: request.userInputId, reason },
    { maxAttempts: 1 },
  );
}
