...
export async function buildReviewPrompt(
  cwd: string,
  command: Extract<WorkflowCommand, { kind: "review" }>,
): Promise<string> {
  const git = async (...args: string[]) =>
    (
      await exec("git", ["-c", "core.quotePath=true", ...args], {
        cwd,
        encoding: "utf8",
  ...
  try {
    ...
    if (command.target === "workingTree") {
      ...
      for (const path of paths) {
        ...
        if (remaining < 0) throw new Error("Review too large");
        ...
        try {
          ...
          if (!stat.isFile()) throw new Error("Unsupported untracked file type");
          if (stat.size > remaining) throw new Error("Review too large");
          ...
          if (length > remaining) throw new Error("Review too large");
          const content = buffer.subarray(0, length);
          if (content.includes(0)) throw new Error("Binary untracked file");
      ...
      const commit = (
        await git("rev-parse", "--verify", "--end-of-options", `${command.ref}^{commit}`)
      ).trim();
    ...
    if (Buffer.byteLength(data) > MAX_REVIEW_BYTES) throw new Error("Review too large");
