...
import { convertPromptContent, formatEmbeddedTextResource, MAX_EMBEDDED_CONTEXT_BYTES, } from "../prompt-content.js";
...
const parse = (value) => JSON.parse(value.slice("Embedded text resource: ".length));
const malformed = [
...
it("round-trips unsaved embedded text and attribution without fetching URIs", () => {
    ...
    try {
        ...
        for (const mimeType of [undefined, null, "", "text/plain"]) {
            ...
            expect(parse(formatEmbeddedTextResource(block)).resource).toEqual(JSON.parse(JSON.stringify(block.resource)));
...
it("bounds aggregate serialized UTF-8 context including metadata and rejects malformed content", () => {
    ...
    expect(convertPromptContent([{ ...embedded, _meta: { big: "a".repeat(MAX_EMBEDDED_CONTEXT_BYTES) } }])
        .ok).toBe(false);
});
it.each(["sdk", "exec"])("forwards embedded context and rejects invalid prompts before %s execution", async (backend) => {
    ...
    try {
        ...
        for (const block of [
            ...
            await expect(ctx.request(methods.agent.session.prompt, { sessionId, prompt: [block] }), JSON.stringify(block).slice(0, 200)).rejects.toMatchObject({ code: -32602 });
        }
        ...
        if (backend === "sdk") {
            ...
            expect(input.map((p) => p.type)).toEqual([
                "text",
                "text",
                "image",
                "text",
        ...
        await expect(ctx.request(methods.agent.session.prompt, { sessionId, prompt: [embedded] })).resolves.toEqual({ stopReason: "end_turn" });
    }
    finally {
