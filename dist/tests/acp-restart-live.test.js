...
const serveHelp = museAvailable() && spawnSync(museCliPath(), ["serve", "--help"], { encoding: "utf8" });
...
describe("ACP process restart continuity (real Muse host)", () => {
    it("requires a local muse serve host for restart proof", () => {
        expect(museReady, "muse serve ≥1.1.1 must be installed; m6 DoD forbids skipping required restart tests").toBe(true);
        expect(existsSync(agentEntrypoint), "dist/index.js must be built before this suite").toBe(true);
    });
    it.each(["load", "resume"])("restarts the ACP process, binds via %s, and continues the same session", async (method) => {
        ...
        try {
            ...
            try {
                ...
                await expect(agent1.ctx.request(methods.agent.session.prompt, {
                    sessionId,
                ...
                expect(provider
                    ...
                    .map((r) => r.model)).toContain(ALTERNATE_MODEL_ID);
                await expect(agent1.ctx.request(methods.agent.session.close, { sessionId })).resolves.toEqual({});
            }
            finally {
            ...
            try {
                ...
                const loaded = await agent2.ctx.request(methods.agent.session[method], {
                    sessionId,
                    cwd,
                    mcpServers: [],
                });
                expect(loaded.configOptions?.find((o) => o.id === "model")?.currentValue).toBe(modelChoice({ id: ALTERNATE_MODEL_ID, name: ALTERNATE_MODEL_ID, providerId: "meta" }));
                expect(loaded.configOptions?.find((o) => o.id === "reasoningEffort")?.currentValue).toBe("medium");
                expect(loaded.modes?.currentModeId).toBe("bypassApprovals");
                expect(loaded.configOptions?.find((o) => o.id === "sandboxNetwork")?.currentValue).toBe("restricted");
                if (method === "load")
                    expect(JSON.stringify(agent2.updates)).toContain("restart-token-one");
                else
                    expect(agent2.updates.some((n) => [
                        "user_message_chunk",
                        "agent_message_chunk",
                        "tool_call",
                        "tool_call_update",
                    ].includes(n.update.sessionUpdate))).toBe(false);
                await expect(agent2.ctx.request(methods.agent.session.prompt, {
                    sessionId,
            ...
            try {
                ...
                try {
                    ...
                    await expectLegacyContinuation(agent3.ctx.request(methods.agent.session.prompt, {
                        sessionId: old.sessionId,
