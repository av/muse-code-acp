...
import { mkdirSync, mkdtempSync, realpathSync, rmSync, symlinkSync, unlinkSync, writeFileSync, readFileSync, } from "node:fs";
...
describe.each(["new", "load", "resume"])("%s workspace validation", (method) => {
    it("validates directories, pins symlinks and leaves state unchanged after rejection", async () => {
        ...
        try {
            ...
            const boundId = "sessionId" in result && typeof result.sessionId === "string"
                ? result.sessionId
                : sessionId;
