...
describe("ACP stdio wire contracts", () => {
    it("uses the built entrypoint over NDJSON for initialize/new/prompt", async () => {
        ...
        try {
            ...
            const requestIds = new Set(parsed.filter((msg) => msg.method && msg.id !== undefined).map((msg) => msg.id));
            ...
            expect(transcript.mspRequests.some((r) => r.method === "turn/start")).toBe(true);
    ...
    it("preserves resource_link prompt content on the MSP boundary", async () => {
        ...
        try {
            ...
            const turnStart = wire
                .getTranscript()
                .mspRequests.find((r) => r.method === "turn/start");
            expect(turnStart.params.input.map((p) => p.text).join("\n\n")).toContain('"uri":"file:///tmp/notes.md"');
    ...
    it("accepts attributed embedded text and rejects binary resources before starting a turn", async () => {
        ...
        try {
            ...
            expect(wire
                .getTranscript()
                .mspRequests.some((r) => r.method === "turn/start")).toBe(false);
            const resource = {
            ...
            const turn = wire
                .getTranscript()
                .mspRequests.find((r) => r.method === "turn/start");
            expect(JSON.parse(turn.params.input[0].text.slice("Embedded text resource: ".length))).toEqual({ resource });
        }
        finally {
            await wire.dispose();
        }
