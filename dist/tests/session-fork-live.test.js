...
describe.skipIf(!available)("native Muse fork continuity", () => {
    it("copies the requested completed history and continues independent branches across ACP restart", async () => {
        ...
        try {
            ...
            const prompt = (id, text) => ctx.request(methods.agent.session.prompt, {
                sessionId: id,
            ...
            await expect(ctx.request(methods.agent.session.fork, {
                sessionId,
                cwd,
            ...
            expect(fork.configOptions?.find((o) => o.id === "model")?.currentValue).toBe(modelChoice({ id: ALTERNATE_MODEL_ID, name: ALTERNATE_MODEL_ID, providerId: "meta" }));
            expect(fork.configOptions?.find((o) => o.id === "reasoningEffort")?.currentValue).toBe("medium");
            ...
            const fullInput = JSON.stringify(provider.requests().findLast((r) => JSON.stringify(r.input).includes("m12-full-branch"))
                ?.input);
            ...
            for (const [id, method] of [
                ...
                expect(restored.configOptions?.find((o) => o.id === "model")?.currentValue).toBe(modelChoice({ id: ALTERNATE_MODEL_ID, name: ALTERNATE_MODEL_ID, providerId: "meta" }));
                expect(restored.configOptions?.find((o) => o.id === "reasoningEffort")?.currentValue).toBe("medium");
                expect(restored.modes?.currentModeId).toBe(id === sessionId ? "readOnly" : "default");
                await expect(restarted.ctx.request(methods.agent.session.prompt, {
                    sessionId: id,
                    prompt: [
                        {
                            type: "text",
            ...
            const branchInput = JSON.stringify(provider.requests().findLast((r) => JSON.stringify(r.input).includes("m12-cut-branch"))
                ?.input);
            expect(branchInput).toContain("m12-source-first");
            expect(branchInput).not.toContain("m12-source-second");
            const sourceInput = JSON.stringify(provider
                .requests()
