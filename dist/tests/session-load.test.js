...
import { connectTestClient, fakeMuseBinary, initialized, museAvailable, silentLogger, } from "./helpers.js";
describe("exportToUpdates", () => {
    const wrap = (payload) => ({ envelope: { payload } });
    ...
    it("rejects a drifted export schema instead of replaying partially", () => {
        ...
        expect(() => exportToUpdates("s1", doc, silentLogger())).toThrow(/unsupported muse export schema/);
    });
...
describe("runMuseExport failure", () => {
    it("rejects with the export error detail", async () => {
        await expect(runMuseExport(crypto.randomUUID(), process.env, fakeMuseBinary())).rejects.toThrow(/muse export exited 2: no session found/);
    });
});
describe.skipIf(!museAvailable())("session list + load round trip (live echo)", () => {
    it("lists a finished session and replays it on load, then continues", async () => {
        ...
        expect(sessions.find((s) => s.sessionId === sessionId)?.title).toContain("repeat token history");
    ...
    it("rejects loading a session that is not in the store", async () => {
        ...
        await expect(ctx.request(methods.agent.session.load, {
