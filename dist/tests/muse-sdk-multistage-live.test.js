...
function stageOf(request) {
    const requirement = request._meta?.museRequirementId;
...
const SCENARIOS = {
    ...
    two: {
        markers: ["a.txt", "b.txt"],
        command: (cwd) => `echo one > ${cwd}/a.txt; ls ${cwd}; echo two > ${cwd}/b.txt; cat ${cwd}/a.txt`,
    },
    three: {
...
async function runScenario(scenario, decide, extended = false) {
    const provider = await startLoopbackProvider({
        ...
        scriptedToolCallForRequest: (request) => {
            ...
            if (!body.includes("a.txt") || body.includes("call_1"))
                return undefined;
            return {
                name: "bash",
...
describe("SDK live multi-stage approvals (real Muse host)", () => {
    it("requires a local muse serve host; skipping is not evidence", () => {
        expect(museReady, "muse serve must be installed; w2/m1 forbids treating a missing host as success").toBe(true);
    });
    it("allows both stages of a two-stage command and writes both files", async () => {
        expect(museReady).toBe(true);
        const result = await runScenario(SCENARIOS.two, (request) => optionOfKind(request, "allow_once"));
    ...
    it("allows all three stages of a three-stage command", async () => {
        expect(museReady).toBe(true);
        const result = await runScenario(SCENARIOS.three, (request) => optionOfKind(request, "allow_once"));
        expect(result.response).toEqual({ stopReason: "end_turn" });
    ...
    it("denies a later stage through a host-offered choice and writes nothing", async () => {
        expect(museReady).toBe(true);
        const result = await runScenario(SCENARIOS.two, (request, index) => index === 0 ? optionOfKind(request, "allow_once") : optionOfKind(request, "reject_once"));
        expect(result.response).toEqual({ stopReason: "end_turn" });
        expect(result.asked).toHaveLength(2);
    ...
    it("reports each stage's refreshed evidence to a negotiating client", async () => {
        expect(museReady).toBe(true);
        const result = await runScenario(SCENARIOS.two, (request) => optionOfKind(request, "allow_once"), true);
        const stages = result.requests.map((request) => (request._meta?.["muse/approval"]).stages ??
            []);
        ...
        expect(stages[1].filter((stage) => stage.resolutionKind === "unresolved").length).toBeLessThan(stages[0].filter((stage) => stage.resolutionKind === "unresolved").length);
    }, 120_000);
});
