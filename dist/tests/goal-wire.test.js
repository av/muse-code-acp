...
it.each([true, false])("goal metadata negotiation=%s preserves baseline inspection, clear and deduplication", async (negotiated) => {
    ...
    try {
        ...
        const prompt = (text) => wire.ctx.request(methods.agent.session.prompt, {
            sessionId,
            prompt: [{ type: "text", text }],
        });
        await prompt("start");
        const goals = () => wire.updates.flatMap((n) => n.update.sessionUpdate === "session_info_update" && n.update._meta?.["muse/goal"]
