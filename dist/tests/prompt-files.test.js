...
function imageBlock(mimeType = "image/png", data = IMAGE_DATA) {
    return { type: "image", mimeType, data };
}
describe("compileMusePrompt", () => {
    it("returns text directly when there are no images", () => {
        return expect(compileMusePrompt([{ type: "text", text: "fix the bug" }])).resolves.toMatchObject({ prompt: "fix the bug", imagePaths: [] });
    });
    ...
    it("rejects images without accompanying text", async () => {
        await expect(compileMusePrompt([imageBlock()])).rejects.toThrow(/requires text or a resource link/);
    });
    it("rejects unsupported content before touching the filesystem", async () => {
        const before = new Set(readdirSync(tmpdir()));
        await expect(compileMusePrompt([{ type: "bogus" }])).rejects.toThrow(/unsupported prompt content/);
        const leaked = readdirSync(tmpdir()).filter((entry) => entry.startsWith("muse-code-acp-images-") && !before.has(entry));
