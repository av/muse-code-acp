...
import { appendFileSync, chmodSync, writeFileSync, readFileSync, existsSync, mkdirSync, rmSync, } from "node:fs";
...
it("allows a real delayed host beyond 20 seconds to complete an ACP turn", async () => {
    ...
    const logs = [];
    const client = connectTestClient({ backend: "sdk", env: f.env }, { log: (s) => logs.push(String(s)), error: (s) => logs.push(String(s)) });
    try {
        ...
        expect(await c.request(methods.agent.session.prompt, {
            sessionId,
...
it("completes 16 concurrent independent ACP clients with isolated owned hosts", async () => {
    ...
    const logs = Array.from({ length: 16 }, () => []);
    const clients = logs.map((log) => connectTestClient({ backend: "sdk", env: f.env }, { log: (s) => log.push(String(s)), error: (s) => log.push(String(s)) }));
    const start = Date.now();
    try {
        const outcomes = await Promise.allSettled(clients.map(async (client, i) => {
            const c = await initialized(client);
        ...
        expect(outcomes.every((o) => o.status === "fulfilled" && o.value.result.stopReason === "end_turn")).toBe(true);
        const ids = outcomes.flatMap((o) => (o.status === "fulfilled" ? [o.value.sessionId] : []));
        expect(new Set(ids).size).toBe(16);
        for (let i = 0; i < 16; i++)
            expect(f.p.requests().some((r) => JSON.stringify(r.input).includes(`concurrent-client-${i}`))).toBe(true);
    }
    finally {
...
it("returns 16 sessions without catalog hosts against populated history and discovers through execution", async () => {
    ...
    const historyDirectories = process.env.MUSE_CODE_ACP_TEST_HISTORY_DIRECTORIES === "6000" ? 6000 : 1000;
    ...
    const clients = Array.from({ length: 16 }, () => connectTestClient({ backend: "sdk", env: f.env }, silentLogger()));
    try {
        const contexts = await Promise.all(clients.map((c) => initialized(c)));
        const start = Date.now();
        const sessions = await Promise.all(contexts.map((c) => c.request(methods.agent.session.new, { cwd: f.p.root, mcpServers: [] })));
        const newSessionMs = Date.now() - start;
        expect(f.pids()).toHaveLength(0);
        expect(newSessionMs).toBeLessThan(5000);
        await Promise.all(contexts.map((c, i) => c.request(methods.agent.session.prompt, {
        ...
        for (const client of clients)
            await expect
                .poll(() => client.updates.some((n) => n.update.sessionUpdate === "config_option_update" &&
