...
describe("listMuseSkills (fake binary)", () => {
    it("parses the skills document", async () => {
        const skills = await listMuseSkills(mkdtempSync(join(tmpdir(), "muse-skills-test-")), process.env, fakeMuseBinary(), silentLogger());
...
describe.skipIf(!museAvailable())("listMuseSkills (real muse)", () => {
    it("lists built-in skills for a fresh workspace", async () => {
        const skills = await listMuseSkills(mkdtempSync(join(tmpdir(), "muse-skills-test-")), process.env, undefined, silentLogger());
