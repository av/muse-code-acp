...
import { capturingLogger, connectTestClient, fakeMuseBinary, initialized, silentLogger, } from "./helpers.js";
function settingsEnv(contents) {
...
describe("readMuseSettings", () => {
    it("reads provider, model, and reasoning effort", () => {
        const env = settingsEnv(JSON.stringify({
            schema_version: 1,
            provider: "meta",
...
describe("defaultSessionConfig", () => {
    it("prefers user settings over built-ins and validates effort", () => {
        expect(defaultSessionConfig({ model: "muse-spark-1.2-contributor", reasoningEffort: "ultra" })).toEqual({ model: "muse-spark-1.2-contributor", reasoningEffort: "ultra" });
...
describe("session config options over ACP", () => {
    ...
    it("advertises options from settings defaults and injects unknown models", async () => {
        const env = settingsEnv(JSON.stringify({ schema_version: 1, model: "muse-spark-9.9-beta", reasoning_effort: "low" }));
        ...
        expect(model?.type === "select" ? model.options.flatMap((o) => ("value" in o ? [o.value] : [])) : []).toContain("muse-spark-1.2");
