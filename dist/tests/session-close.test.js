...
import { connectTestClient, fakeMuseBinary, museAvailable, newTestSession, silentLogger, } from "./helpers.js";
function capturePath() {
    return join(mkdtempSync(join(tmpdir(), "muse-close-capture-")), "capture.json");
}
function blockingClient(capture) {
...
describe("session/close", () => {
    ...
    it.each(["close", "dispose"])("waits for active turn and image cleanup on %s", async (method) => {
        ...
        try {
            ...
            await expect
                .poll(() => testClient.updates.some((n) => n.update.sessionUpdate === "agent_message_chunk"), { timeout: 5000 })
                .toBe(true);
    ...
    it("acknowledges close when the active prompt rejects during unwind", async () => {
        const updateEntered = Promise.withResolvers();
    ...
    it.skipIf(!museAvailable())("preserves the native Muse session log", async () => {
