    ...
    it("shows observed preimages, keeps reports partial, and preserves baseline diffs without extra model turns", async () => {
        ...
        const provider = await startLoopbackProvider({
            ...
            scriptedToolCallForRequest: (request) => {
                ...
                if (!text.includes("m13-observed-change"))
                    return;
                if (stage++ === 0)
                    return {
                        name: "write_file",
        ...
        try {
            ...
            const diffs = first.updates.flatMap((n) => "content" in n.update && Array.isArray(n.update.content)
                ? n.update.content.filter((c) => c.type === "diff")
                : []);
            expect(diffs).toContainEqual(expect.objectContaining({ oldText: "dirty user preimage", newText: "agent overwrite" }));
            expect(diffs).toContainEqual(expect.objectContaining({ oldText: null, newText: "agent create" }));
            ...
            expect(second.updates.flatMap((n) => "content" in n.update && Array.isArray(n.update.content) ? n.update.content : [])).toContainEqual(expect.objectContaining({
                type: "diff",
