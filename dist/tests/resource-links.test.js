...
const parse = (text) => JSON.parse(text.slice("Resource link: ".length));
it("round-trips fields without conflating null, absent and zero", () => {
    ...
    try {
        ...
        expect(parse(formatResourceLink({ type: "resource_link", name: "n", uri: "file:///missing" }))).toEqual({ name: "n", uri: "file:///missing" });
...
it.each(["sdk", "exec"])("forwards complete resource context through %s", async (backend) => {
    ...
    try {
        ...
        await expect(ctx.request(methods.agent.session.prompt, { sessionId, prompt: [resource] })).resolves.toEqual({ stopReason: "end_turn" });
    }
    finally {
