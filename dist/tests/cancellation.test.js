...
function sdkClient(mode, barrier) {
...
describe("SDK cancellation races", () => {
    it.each(["handshake", "session", "ack"])("cancels during %s barrier and settles as cancelled", async (barrier) => {
    ...
    it("cancels during streaming and allows a second prompt after recovery", async () => {
        ...
        await expect(next.ctx.request(methods.agent.session.prompt, {
