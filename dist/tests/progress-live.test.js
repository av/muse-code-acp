...
it("reports root usage, public summaries and restored status without another model call", async () => {
    ...
    try {
        ...
        const usage = () => client.updates.flatMap((n) => n.update.sessionUpdate === "session_info_update" && n.update._meta?.["muse/usage"]
            ? [n.update._meta["muse/usage"]]
            : []);
        expect(usage().length).toBeGreaterThan(0);
        const lastUsage = usage().at(-1);
        const summaries = client.updates
            .flatMap((n) => n.update.sessionUpdate === "agent_thought_chunk" && n.update.content.type === "text"
...
it("renders native todo create/modify/complete/clear and restores the latest snapshot", async () => {
    ...
    try {
        ...
        const plans = () => client.updates.flatMap((n) => (n.update.sessionUpdate === "plan" ? [n.update.entries] : []));
        for (const state of ["pending", "in_progress", "completed", "clear"]) {
            await ctx.request(methods.agent.session.prompt, {
                sessionId,
                prompt: [{ type: "text", text: `todo-live-marker:${state}` }],
            });
            expect(plans().at(-1)).toEqual(state === "clear"
                ? []
...
it("streams correlated shell output before completion with a bounded final snapshot", async () => {
    ...
    try {
        ...
        await expect
            .poll(() => client.updates.some((n) => (n.update.sessionUpdate === "tool_call" ||
            n.update.sessionUpdate === "tool_call_update") &&
            n.update.status === "in_progress" &&
            JSON.stringify(n.update.content).includes("first-output")), { timeout: 10000 })
            .toBe(true);
        await done;
        const tools = client.updates.flatMap((n) => n.update.sessionUpdate === "tool_call" || n.update.sessionUpdate === "tool_call_update"
            ? n.update.name === "bash"
                ? [n.update]
                : []
            : []);
