...
describe("session/resume", () => {
    it("preserves live mode and config while replacing MCP servers", async () => {
        ...
        expect(resumed.configOptions?.find((option) => option.id === "mode")?.currentValue).toBe("readOnly");
        expect(resumed.configOptions?.find((option) => option.id === "reasoningEffort")?.currentValue).toBe("low");
    ...
    it("accepts a symlink-equivalent workspace", async () => {
        ...
        await expect(ctx.request(methods.agent.session.resume, { sessionId, cwd: linkedCwd, mcpServers: [] })).resolves.toMatchObject({ modes: expect.any(Object) });
    });
    it("rejects workspace changes with the stored path and next step", async () => {
        ...
        await expect(ctx.request(methods.agent.session.resume, { sessionId, cwd: otherCwd, mcpServers: [] })).rejects.toMatchObject({
            code: -32602,
    ...
    it("rejects a missing requested workspace as invalid input", async () => {
        ...
        await expect(ctx.request(methods.agent.session.resume, { sessionId, cwd: missingCwd, mcpServers: [] })).rejects.toMatchObject({
            code: -32602,
    ...
    it("rejects a regular file used as a workspace", async () => {
        ...
        await expect(client.agent.resumeSession({ sessionId, cwd: file, mcpServers: [] })).rejects.toMatchObject({ code: -32602 });
    });
    it("rejects a deleted stored workspace with a new-session next step", async () => {
        const testClient = fakeClient();
        const { ctx, sessionId, cwd } = await newTestSession(testClient);
        rmSync(cwd, { recursive: true, force: true });
        await expect(ctx.request(methods.agent.session.resume, { sessionId, cwd, mcpServers: [] })).rejects.toMatchObject({
            code: -32602,
    ...
    it("rejects additional workspace directories visibly", async () => {
        const testClient = fakeClient();
        const { ctx, sessionId, cwd } = await newTestSession(testClient);
        await expect(ctx.request(methods.agent.session.resume, {
            sessionId,
            cwd,
    ...
    it.skipIf(!museAvailable())("recreates a closed session without replaying history", async () => {
        ...
        expect(testClient.updates.some((update) => update.update.sessionUpdate === "agent_message_chunk")).toBe(false);
