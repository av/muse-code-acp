...
describe("MCP passthrough", () => {
    ...
    it("merges session MCP with user settings in a private disposable overlay", () => {
        ...
        writeFileSync(join(museDir, "settings.json"), JSON.stringify({
            schema_version: 1,
            provider: "meta",
            mcp_servers: {
        ...
        const settings = JSON.parse(readFileSync(join(overlay.configHome, "muse", "settings.json"), "utf8"));
    ...
    it("injects the overlay for a prompt and removes it after Muse exits", async () => {
        ...
        writeFileSync(join(configHome, "muse", "settings.json"), '{"schema_version":1,"model":"original-model"}\n');
    ...
    it("rejects unsupported SSE MCP transports", () => {
        ...
        ], process.env)).toThrow(/unsupported MCP transport/);
    });
});
it("merges canonical and legacy global servers with session HTTP precedence in an isolated SDK overlay", () => {
    ...
    ], { XDG_CONFIG_HOME: root }, { model: "muse-spark-1.2", reasoningEffort: "high" });
    try {
        const settings = JSON.parse(readFileSync(join(overlay.configHome, "muse", "settings.json"), "utf8"));
