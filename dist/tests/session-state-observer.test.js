...
import { SessionStateObserver, SESSION_STATE_EXTENSION, } from "../session-state-observer.js";
import { connectTestClient, fixturesDir, newTestSession } from "./helpers.js";
it.each([false, true])("reports observed state and post-resolve persistence over ACP (negotiated: %s)", async (negotiated) => {
    ...
    try {
        const { ctx, sessionId } = await newTestSession(client, negotiated ? { _meta: { [SESSION_STATE_EXTENSION]: 1 } } : {});
        await expect(ctx.request(methods.agent.session.prompt, {
            sessionId,
            prompt: [{ type: "text", text: "observe" }],
        })).resolves.toEqual({ stopReason: "end_turn" });
        const reports = () => client.updates.flatMap((n) => n.update._meta?.[SESSION_STATE_EXTENSION]
...
it("keeps absent state absent, reports clears, and waits for a current fold", async () => {
    const seen = [];
    const observer = new SessionStateObserver("s", async (value) => {
        seen.push(value);
    }, () => { });
    const values = new Map();
    const fold = {
        current: true,
        sessionState: { get: (key) => values.get(key) },
    };
    const connection = {};
...
it("times out observation without retrying or publishing a late result after stop", async () => {
    ...
    const reports = [];
    let resolve;
    const request = vi.fn(() => new Promise((r) => {
        resolve = r;
    }));
    const observer = new SessionStateObserver("s", async (value) => {
        reports.push(value);
    ...
    try {
        ...
        const poll = observer.poll(new SessionFold(), { request });
