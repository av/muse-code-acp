...
function parseAll(text) {
    const envelopes = [];
    const garbage = [];
    const parser = new MuseLineParser((envelope) => envelopes.push(envelope), (line, reason) => garbage.push([line, reason]));
    parser.push(text);
    parser.end();
...
describe("MuseLineParser", () => {
    ...
    it("parses a recorded echo run into ordered envelopes", () => {
        ...
        expect(envelopes.map((e) => e.sequence)).toEqual([...envelopes].map((_, i) => envelopes[0].sequence + i));
