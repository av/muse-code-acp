...
it("runs skills, rename and logout locally without a model turn", async () => {
    ...
    try {
        ...
        const prompt = (text) => w.ctx.request(methods.agent.session.prompt, { sessionId, prompt: [{ type: "text", text }] });
        await prompt("/skills");
        await prompt("/rename User title 中文");
        expect(w.updates.some((n) => n.update.sessionUpdate === "session_info_update" && n.update.title === "User title 中文")).toBe(true);
        await prompt("/logout");
        expect(JSON.stringify(w.updates)).toContain("Credentials remain configured");
        expect(w.getTranscript().mspRequests.filter((r) => r.method === "turn/start")).toHaveLength(0);
...
it("compatible steering snapshots the active turn and never falls back to a new turn", async () => {
    ...
    try {
        ...
        const steer = (extra = {}) => w.ctx.request("_session/steering", {
            sessionId,
        ...
        await running;
        await expect(steer()).rejects.toMatchObject({ code: -32600 });
        expect(w.getTranscript().mspRequests.filter((r) => r.method === "turn/start")).toHaveLength(1);
    }
    finally {
        await w.dispose();
    }
