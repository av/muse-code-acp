...
it("traverses 123 public rows in bounded pages without acquiring a writer lease", async () => {
    ...
    try {
        ...
        await expect(wire.ctx.request(methods.agent.session.list, {
            cwd: wire.workspace,
            cursor: first.nextCursor,
        })).rejects.toMatchObject({ code: -32602 });
        await expect(wire.ctx.request(methods.agent.session.list, { cwd: root, cursor: "bad!" })).rejects.toMatchObject({ code: -32602 });
        const requests = wire.getTranscript().mspRequests;
        expect(requests.some((r) => ["session/start", "session/resume", "turn/start"].includes(r.method))).toBe(false);
    }
    finally {
        await wire.dispose();
    }
...
it("disposal stops an outstanding discovery host and waits for cleanup", async () => {
    ...
    roots.push(root);
    const capture = join(root, "requests.jsonl"), pid = join(root, "pid");
