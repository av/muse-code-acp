...
export function envelope(payloadType, payload) {
    return {
        schema_version: 1,
        id: "test-id",
...
describe("TurnTranslator text deltas", () => {
    it("maps run.output.delta to an agent_message_chunk", () => {
        const updates = translator().toUpdates(envelope("run.output.delta", { kind: "run_output_delta", text: "hi there" }));
    ...
    it("drops empty text deltas", () => {
        const updates = translator().toUpdates(envelope("run.output.delta", { kind: "run_output_delta", text: "" }));
        expect(updates).toEqual([]);
    });
    it("drops malformed run.output.delta payloads instead of throwing", () => {
        const updates = translator().toUpdates(envelope("run.output.delta", { kind: "run_output_delta", text: 42 }));
        expect(updates).toEqual([]);
    });
