...
it("negotiates steering, serializes corrections into captured turn and retains baseline busy rejection", async () => {
    ...
    try {
        ...
        const steer = (text) => wire.ctx.request(STEER_METHOD, {
            sessionId,
            expectedTurnId,
        ...
        await expect(wire.ctx.request(STEER_METHOD, {
            sessionId,
        ...
        expect(requests.filter((r) => r.method === "turn/steer").map((r) => r.params.input?.[0].text)).toEqual(["first", "second"]);
...
it("close settles an in-flight steering request and rejects its queued successor without replay", async () => {
    ...
    try {
        ...
        await expect.poll(() => activeId(wire.updates)).toEqual(expect.any(String));
        const expectedTurnId = activeId(wire.updates);
        const results = ["first", "queued"].map((text) => wire.ctx
            .request(STEER_METHOD, { sessionId, expectedTurnId, prompt: [{ type: "text", text }] })
            .then(() => "accepted", () => "rejected"));
        await expect
            .poll(() => wire
            .getTranscript()
            .mspRequests.some((r) => r.method === "turn/steer"))
            .toBe(true);
        ...
        expect(wire
            .getTranscript()
            .mspRequests.filter((r) => r.method === "turn/steer")).toHaveLength(1);
    }
    finally {
        await wire.dispose();
    }
