...
async function fixture() {
    ...
    writeFileSync(settingsPath, JSON.stringify({ model: "original-model", provider: "original-provider" }));
    ...
    const create = (credential) => ctx.request(methods.agent.session.new, { cwd: root, mcpServers: [server(credential)] });
    const prompt = async (sessionId) => {
        ...
        expect(client.updates
            .slice(before)
            .flatMap(({ update }) => update.sessionUpdate === "agent_message_chunk" && update.content.type === "text"
            ? [update.content.text]
            : [])
...
it("reuses a session host across turns, isolates another session and cleans retained overlays", async () => {
    ...
    try {
        ...
        expect(JSON.parse(readFileSync(join(firstHome, "muse", "settings.json"), "utf8")).mcpServers["session-mcp"].env.TOKEN).toBe("first-secret");
...
it.each(["effort", "readOnly", "mcp", "settings", "auth"])("applies %s changes with only necessary host replacement", async (change) => {
    const f = await fixture();
    try {
        ...
        if (change === "effort") {
            ...
            writeFileSync(f.settingsPath, JSON.stringify({ provider: "fresh-provider", model: "original-model" }));
        }
        else {
        ...
        if (change === "effort")
            expect(f
                ...
                .at(-1).params.reasoningEffort).toBe("minimal");
        if (change === "readOnly") {
        ...
        if (change === "settings")
            expect(capture.settings.provider).toBe("original-provider");
        if (change === "auth")
            expect(JSON.parse(readFileSync(join(capture.configHome, "muse", "auth.json"), "utf8"))
                .credential).toBe("fresh-auth");
...
it("updates the execution model with a public setter even when resume metadata already agrees", async () => {
    ...
    try {
        ...
        expect(f
            ...
            .at(-1).params.model).toMatchObject({ modelId: "new-model", providerId: "original-provider" });
    }
    finally {
        await f.close();
    }
