...
function responseFrame(id, status, extra = {}) {
    return { id, object: "response", model: FAKE_MODEL_ID, status, output: [], ...extra };
}
function textHead(text, summary) {
    return (sse({
...
function toolCallHead(callId, tool) {
    return (sse({
        type: "response.created",
        sequence_number: 1,
...
export async function startLoopbackProvider(options) {
    ...
    const server = createServer((request, response) => {
        ...
        request.on("end", async () => {
            ...
            const requestedTool = options.scriptedToolCallForRequest
                ...
                : scriptedTool;
            const offersScriptedTool = (parsed.tools ?? []).some((namespace) => namespace.tools
                ? namespace.tools.some((tool) => tool.name === requestedTool?.name ||
                    `${namespace.name}__${tool.name}` === requestedTool?.name)
                : namespace.name === requestedTool?.name);
            const isScripted = !!requestedTool &&
