...
describe("SDK live approval gating (real Muse host)", () => {
    it("requires a local muse serve host for execution-gating proof", () => {
        expect(museReady, "muse serve ≥1.1.1 must be installed; m5 DoD forbids skipping required approval effect tests").toBe(true);
    });
    it("allow writes the marker", async () => {
        ...
        try {
            ...
            await expect(ctx.request(methods.agent.session.setMode, { sessionId, modeId: "bypassApprovals" })).rejects.toMatchObject({ code: -32600 });
            ...
            expect(allow, `expected allow among ${JSON.stringify(client.permissionRequests[0].options)}`).toBeTruthy();
            release({ outcome: { outcome: "selected", optionId: allow.optionId } });
            await expect(prompt).resolves.toEqual({ stopReason: "end_turn" });
            expect(existsSync(marker), `approved command did not write its marker; tool updates: ${JSON.stringify(client.updates.filter(({ update }) => update.sessionUpdate === "tool_call_update"))}`).toBe(true);
            expect(readFileSync(marker, "utf8").trim()).toBe("yes");
            expect(client.updates.some((n) => n.update.sessionUpdate === "session_info_update" &&
                n.update._meta?.["muse/approval"]?.decision === "approved")).toBe(true);
    ...
    it.each(["cancel", "close"])("%s while pending blocks stale approval; a later prompt works", async (method) => {
        expect(museReady).toBe(true);
        const provider = await startLoopbackProvider({
            scriptedToolCallWhen: [ARTIFACT, `"bash"`],
            scriptedToolCallCommand: COMMAND,
