...
function fakeStore() {
    ...
    const write = (sessionId, workspace, prompt, day = "2026/08/25", extraLines = []) => {
...
describe("listStoredSessions", () => {
    ...
    it("truncates long titles and reads only the log head on large files", () => {
        ...
        const filler = Array.from({ length: 2000 }, (_, i) => JSON.stringify({ padding: "x".repeat(1000), line: i }));
    ...
    it("returns empty on a missing store", () => {
        expect(listStoredSessions(null, { XDG_DATA_HOME: "/nonexistent-store" }, silentLogger())).toEqual([]);
    });
});
