...
import { chmodSync, copyFileSync, mkdtempSync, mkdirSync, readFileSync, rmSync, writeFileSync, appendFileSync, } from "node:fs";
...
function setup(options = {}) {
    ...
    const setModels = (ids) => writeFileSync(catalog, JSON.stringify({
        source: "fakeCatalog",
...
test("retains duplicate model names across distinct provider identities", async () => {
    const f = setup();
    writeFileSync(f.catalog, JSON.stringify({
        source: "fakeCatalog",
        models: [
...
test("borrowed catalog waiters share one request and cancelling one does not close the host", async () => {
    ...
    const close = vi.fn();
    const connection = { request, close };
