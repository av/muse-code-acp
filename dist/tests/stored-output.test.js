...
import { OUTPUT_EXTENSION, parseOutputRequest, validateOutputPage, readStoredOutput, } from "../stored-output.js";
...
it("verifies public ownership and availability before fetching, without following URIs", async () => {
    ...
    const request = vi.fn(async (method) => method === "view/page" ? { events: [{ params: { sessionId, item: observed } }] } : page);
...
it("propagates a vanished output failure after one public read, with no retries", async () => {
    const request = vi.fn(async (method) => {
        if (method === "view/page")
            return { events: [{ params: { sessionId: "session", item } }] };
        throw new Error("outputUnavailable");
    });
    await expect(readStoredOutput({ request }, params)).rejects.toThrow("outputUnavailable");
