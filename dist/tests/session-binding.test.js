...
describe("session binding races", () => {
    it("rejects close, prompt and duplicate load while history is being read", async () => {
        ...
        try {
            ...
            await expect(client.agent.prompt({ sessionId, prompt: [{ type: "text", text: "race" }] })).rejects.toMatchObject({ code: -32600 });
            await expect(client.agent.loadSession({ sessionId, cwd, mcpServers: [] })).rejects.toMatchObject({ code: -32600 });
...
it.each(["cancel", "close", "dispose"])("suppresses delayed goal inspection delivery after %s", async (method) => {
    const client = connectTestClient({
        backend: "sdk",
        museBinary: fakeMuseBinary(),
    ...
    try {
        ...
        await stopping;
        expect(update.mock.calls.filter(([notification]) => notification.update.sessionUpdate === "agent_message_chunk" ||
            notification.update.sessionUpdate === "session_info_update")).toEqual([]);
        if (method === "cancel")
...
describe("disposal during binding", () => {
    it.each(["load", "resume"])("does not restore state after a delayed %s", async (method) => {
        const client = connectTestClient({
        ...
        const exportGate = Promise.withResolvers();
        ...
        try {
            const binding = (method === "load"
                ? client.agent.loadSession({ sessionId, cwd, mcpServers: [] })
                : client.agent.resumeSession({ sessionId, cwd, mcpServers: [] })).then(() => "restored", () => "rejected");
            ...
            await expect(client.agent.prompt({ sessionId, prompt: [{ type: "text", text: "late" }] })).rejects.toMatchObject({ code: -32600 });
            await expect(client.agent.loadSession({ sessionId, cwd, mcpServers: [] })).rejects.toMatchObject({ code: -32600 });
            await expect(client.agent.resumeSession({ sessionId, cwd, mcpServers: [] })).rejects.toMatchObject({ code: -32600 });
            await client.agent.dispose();
        }
        finally {
            read.mockRestore();
...
it("waits for an in-flight replay update and stops the rest on disposal", async () => {
    ...
    try {
        const binding = client.agent.loadSession({ sessionId, cwd, mcpServers: [] }).then(() => "loaded", () => "rejected");
...
it("cancelling while review-start delivery waits emits a cancelled terminal without starting a turn", async () => {
    ...
    const statuses = [];
    const update = vi.spyOn(client.agent.client, "sessionUpdate").mockImplementation(async (n) => {
        const status = n.update.sessionUpdate === "session_info_update"
            ? n.update._meta?.["muse/review"]?.status
            : undefined;
        if (status)
