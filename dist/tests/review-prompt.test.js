...
it("reviews exact staged, unstaged and untracked data without changing the worktree", async () => {
    ...
    for (const text of ["staged", "unstaged", "untracked", "new.txt"])
        expect(prompt).toContain(text);
    expect(git("status", "--porcelain")).toBe(before);
});
it("branch and commit reviews use resolved commits and reject invalid option-like refs", async () => {
    const { cwd } = repo();
    expect(await buildReviewPrompt(cwd, { kind: "review", target: "branch", ref: "base" })).toContain("+committed");
    expect(await buildReviewPrompt(cwd, { kind: "review", target: "commit", ref: "HEAD" })).toContain("+committed");
    await expect(buildReviewPrompt(cwd, { kind: "review", target: "commit", ref: "missing" })).rejects.toMatchObject({ code: -32602 });
});
it("rejects unsafe untracked types and oversized snapshots explicitly", async () => {
    const { cwd } = repo();
    symlinkSync("a.txt", join(cwd, "link"));
    await expect(buildReviewPrompt(cwd, { kind: "review", target: "workingTree" })).rejects.toMatchObject({ code: -32602 });
