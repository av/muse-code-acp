...
it.each([
...
])("real automatic policy and filesystem effects: %j", async ({ mode, outside, sandbox, succeeds, policy }) => {
    ...
    const provider = await startLoopbackProvider({
        ...
        holdMs: 20,
        scriptedToolCallForRequest: (r) => JSON.stringify(r).includes("call_1")
            ? undefined
    ...
    try {
        ...
        if (policies?.type === "select") {
            expect(policies.options.some((o) => "value" in o && o.value === "allowAll")).toBe(probeSdkHost().version !== "1.1.1");
        }
        else
            throw Error("Missing native policy choices");
...
it.each(["proxy-only", "restricted", "enabled"])("records loopback network effects for %s", async (network) => {
    let hits = 0;
...
it.each(["enabled", "disabled"])("non-shell workspace writes are %s", async (value) => {
    const provider = await startLoopbackProvider({
        scriptedToolCallWhen: ["unused"],
        scriptedToolCallCommand: "unused",
        holdMs: 20,
