...
it("planning mode cannot change during a turn and prompt text cannot disable its host flags", async () => {
    ...
    try {
        ...
        await expect(wire.ctx.request(methods.agent.session.setMode, { sessionId, modeId: "default" })).rejects.toMatchObject({ code: -32600 });
        expect(JSON.stringify(wire.getTranscript().mspRequests)).toContain("Only an explicit client mode change");
        await wire.ctx.notify(methods.agent.session.cancel, { sessionId });
        expect(await running).toEqual({ stopReason: "cancelled" });
        await expect(wire.ctx.request(methods.agent.session.setMode, { sessionId, modeId: "default" })).resolves.toEqual({});
    }
    finally {
        await wire.dispose();
    }
