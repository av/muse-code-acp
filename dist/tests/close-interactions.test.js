...
describe.each(["approval", "userInput"])("close during %s", (mode) => {
    it.each(["closed", "resumed"])("ignores replies arriving when %s", async (lateAt) => {
        ...
        const permissionGates = Array.from({ length: 3 }, () => Promise.withResolvers());
        const inputGates = Array.from({ length: 3 }, () => Promise.withResolvers());
        let permissionIndex = 0, inputIndex = 0;
        const oldReturned = Promise.withResolvers();
        ...
        const release = (index, stale) => {
            if (mode === "approval")
                permissionGates[index].resolve({
                    outcome: { outcome: "selected", optionId: stale ? "allow-once" : "deny-once" },
                });
            else
                inputGates[index].resolve({ action: "accept", content: { q1: stale ? "red" : "blue" } });
        };
        ...
        const prompt = (id) => ctx.request(methods.agent.session.prompt, {
            sessionId: id,
