...
it("handles /mcp locally, reserves its name and rejects malformed inputs before session creation", async () => {
    ...
    try {
        ...
        await expect
            .poll(() => client.updates.some((n) => n.update.sessionUpdate === "available_commands_update" &&
        ...
        expect(client.agent.sessions.get(sessionId)?.sdkHost).toBeUndefined();
        await expect(ctx.request(methods.agent.session.new, {
            cwd: root,
...
it("close waits for a local MCP response and prevents another prompt from overtaking it", async () => {
    ...
    try {
        ...
        await expect(client.agent.prompt({ sessionId, prompt: [{ type: "text", text: "/mcp" }] })).rejects.toMatchObject({ code: -32600 });
        let closed = false;
