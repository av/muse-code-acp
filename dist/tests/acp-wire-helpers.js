 ...
 */
export async function createWireFixture(options = {}) {
    ...
    const [stdoutForClient, stdoutForRecord] = Readable.toWeb(child.stdout).tee();
    void recordLines(stdoutForRecord, acpInbound);
    let agentStdin = Writable.toWeb(child.stdin);
    if (options.fragmentWrites) {
        const writer = agentStdin.getWriter();
        agentStdin = new WritableStream({
            async write(chunk) {
    ...
    const ctx = await Promise.race([
        ctxPromise,
        new Promise((_, reject) => setTimeout(() => reject(new Error(`wire connect timeout\nstderr:\n${stderr}`)), 10_000)),
    ]);
...
async function recordLines(stream, into) {
