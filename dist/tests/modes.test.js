...
describe("session/set_mode", () => {
    ...
    it("rejects unknown or unavailable modes", async () => {
        const testClient = connectTestClient({ backend: "exec", museBinary: fakeMuseBinary() });
        const { ctx, sessionId } = await newTestSession(testClient);
        await expect(ctx.request(methods.agent.session.setMode, { sessionId, modeId: "yolo" })).rejects.toMatchObject({ code: -32602 });
        await expect(ctx.request(methods.agent.session.setMode, { sessionId, modeId: "no-such-mode" })).rejects.toMatchObject({ code: -32602 });
    });
});
