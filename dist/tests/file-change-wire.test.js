...
function reports(wire) {
    return wire.updates.flatMap((n) => {
        const meta = n.update._meta;
...
it("negotiates reports, emits one before the prompt response, and never starts an audit turn", async () => {
    ...
    try {
        ...
        const reportIndex = frames.findIndex((f) => f.params?.update?._meta?.jetbrains?.air?.agentFileChangeReport);
        expect(reportIndex).toBeGreaterThan(-1);
        expect(reportIndex).toBeLessThan(frames.findIndex((f) => f.result?.stopReason));
        expect(frames.find((f) => f.result?.protocolVersion)?.result._meta.jetbrains.air.capabilities).toContain("agentFileChangeReport");
        expect(wire
            .getTranscript()
            .mspRequests.filter((r) => r.method === "turn/start")).toHaveLength(1);
...
it.each(["cancel", "close"])("settles a requested report as unavailable on %s without replay", async (method) => {
    const wire = await createWireFixture({
        fakeMspMode: "block",
        clientCapabilities: { _meta: FILE_REPORT_CAPABILITIES },
    });
...
it.each([false, true])("ignores unnegotiated or malformed requests (negotiated: %s)", async (negotiated) => {
    const wire = await createWireFixture({
