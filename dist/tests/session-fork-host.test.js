...
it.each(["active", "unknown-active", "workspace"])("rejects %s source before a native fork", async (kind) => {
    if (kind === "active")
        source.activeTurnId = "live-turn";
    if (kind === "unknown-active")
        delete source.activeTurnId;
    if (kind === "workspace")
        source.workspaceRoot = tmpdir();
    await expect(forkMuseSession(options)).rejects.toThrow();
    expect(mocks.command).toHaveBeenCalledTimes(1);
    expect(mocks.close).toHaveBeenCalledOnce();
});
it.each(["identity", "model", "provenance", "boundary"])("rejects inconsistent fork %s", async (kind) => {
    if (kind === "identity")
        fork.sessionId = "source";
    if (kind === "model")
        fork.modelId = "different";
    if (kind === "provenance")
        fork.forkedFrom = null;
    if (kind === "boundary")
...
it("preserves fork timeout certainty and never retries a possibly created branch", async () => {
    vi.useFakeTimers();
    try {
        mocks.command.mockImplementation(async (method) => method === "session/read" ? { session: source, pendingRequests: [] } : new Promise(() => { }));
        ...
        await assertion;
        expect(mocks.command.mock.calls.filter(([method]) => method === "session/fork")).toHaveLength(1);
    }
    finally {
        vi.useRealTimers();
    }
