...
const captured = (promise) => promise.then((value) => ({ ok: true, value }), (error) => ({ ok: false, error }));
it("dispatches concurrent corrections in FIFO order to the captured target and snapshots content", async () => {
    ...
    if (secondInput[0].type === "text")
        secondInput[0].text = "mutated";
...
it.each(["replacement", "cancel"])("rejects queued old-turn requests after %s without retargeting", async (reason) => {
    const target = { id: "turn-1" };
    let current = target;
    let cancelled = false;
...
it("bounds queued corrections while one acknowledgement is pending", async () => {
    ...
    const waiting = Array.from({ length: 17 }, () => queue.enqueue("turn", "id", [{ type: "text", text: "correction" }]).catch(() => "closed"));
    await expect(queue.enqueue("turn", "id", [{ type: "text", text: "overflow" }])).rejects.toMatchObject({ code: -32600 });
    queue.close();
