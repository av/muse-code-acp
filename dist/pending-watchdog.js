...
export function stallLimitMs(env = process.env) {
...
export function inputLimitMs(env = process.env) {
 ...
 */
export const DEFAULT_TOOL_IDLE_MS = 900_000;
/** Read the configured in-progress-tool bound; invalid values keep the default. */
export function toolIdleMs(env = process.env) {
    const raw = Number(env.MUSE_CODE_ACP_TOOL_IDLE_MS);
    return Number.isFinite(raw) && raw > 0 ? raw : DEFAULT_TOOL_IDLE_MS;
}
/**
...
export class TurnSilenceWatchdog {
    ...
    now;
    since;
    constructor(limitMs = DEFAULT_TURN_IDLE_MS, now = Date.now) {
...
export class PendingWorkWatchdog {
    ...
    inputLimitMs;
    seen = new Map();
    constructor(limitMs = DEFAULT_STALL_LIMIT_MS, now = Date.now, inputLimitMs = DEFAULT_INPUT_LIMIT_MS) {
        this.limitMs = limitMs;
        this.now = now;
        this.inputLimitMs = inputLimitMs;
    }
    /**
