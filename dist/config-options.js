...
export function defaultSessionConfig(settings) {
    return {
    ...
    return [
        {
            ...
            category: "model",
            type: "select",
            currentValue: modelChoice(current.length === 1
                ? current[0]
                : {
                    id: config.model,
                    name: config.model,
...
export function applyConfigSelection(config, configId, value, discovery) {
 ...
 */
function modelOptionName(model, all) {
    const ambiguous = all.some((other) => other.name === model.name && modelChoice(other) !== modelChoice(model));
    if (!model.providerId || !ambiguous)
        return model.name;
...
export function selectModel(config, value, discovery) {
    ...
    if (matches.length > 1)
        throw RequestError.invalidParams(undefined, "Ambiguous model ID; select a provider-qualified catalog choice");
    if (matches.length === 1) {
        const model = matches[0];
        if (model.profileId != null)
            throw RequestError.invalidParams(undefined, "Named model profile routing is unverified on supported Muse hosts; choose a provider model without a named profile");
        return { ...config, model: model.id, providerId: model.providerId, profileId: model.profileId };
    }
    if (value.startsWith("muse-model:") &&
        value !==
            modelChoice({
                id: config.model,
                name: config.model,
        ...
        throw RequestError.invalidParams(undefined, "Model choice is no longer in the catalog; refresh the available choices");
...
export function effortDescription(backend, hostVersion) {
    if (backend === "exec")
...
export function resolvedModel(config, discovery, defaultProvider = "meta") {
    const candidates = discovery?.status === "available"
        ? discovery.models.filter((model) => model.id === config.model &&
            (!config.providerId || model.providerId === config.providerId) &&
            (config.profileId === undefined || (model.profileId ?? null) === config.profileId))
        : [];
    if (candidates.length > 1)
        throw RequestError.invalidParams(undefined, "Current model has ambiguous provider/profile identity; select a qualified catalog choice");
    return {
        model: config.model,
