...
export async function compileMusePrompt(blocks) {
    const converted = convertPromptContent(blocks);
    if (!converted.ok)
        throw converted.error;
    const images = converted.parts.flatMap((part) => part.type === "image"
        ? [
            {
    ...
    if (!prompt)
        throw RequestError.invalidParams(undefined, "Muse Code requires text or a resource link alongside image content");
    if (images.length === 0) {
