...
export function foldedProgress(fold) {
...
export async function readProgress(connection, sessionId) {
    const result = {};
    let cursor;
    const seen = new Set();
    for (let pageNumber = 0; pageNumber < 20; pageNumber++) {
        ...
        for (const event of [...page.events].reverse()) {
            if (event.params?.sessionId !== sessionId ||
                !families.includes(event.method))
                continue;
            const family = event.method;
            if (!(family in result))
                result[family] = event.params;
        }
        if (families.every((f) => f in result) ||
...
export class SessionProgress {
    ...
    signatures = new Map();
    facts = {};
    constructor(sessionId, negotiated, publish) {
        this.sessionId = sessionId;
        this.negotiated = negotiated;
        this.publish = publish;
    }
    async observe(values, reset = false) {
        if (reset) {
            this.facts = {};
        ...
        for (const family of families) {
            ...
            if (data?.sessionId !== undefined && data.sessionId !== this.sessionId)
                continue;
            ...
            if (family === "session/tokenUsage") {
                            ...
                            [USAGE_EXTENSION]: {
                                ...
                                lastModelCall: {
                                    ...
                                    usage: record(data?.usage)
                                        ? Object.fromEntries(Object.entries(record(data?.usage)).filter(([key, value]) => [
                                            "inputTokens",
                                            "outputTokens",
                                            "cachedTokens",
                                            "cacheReadTokens",
                ...
                const { usedTokens, windowTokens } = canonical;
                ...
                if (value !== null && !Array.isArray(data?.items))
                    continue;
            ...
            if (this.signatures.get(family) === signature)
                continue;
            this.signatures.set(family, signature);
            if (update)
                await this.publish({ sessionId: this.sessionId, update });
        }
