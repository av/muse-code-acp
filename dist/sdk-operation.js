...
import { sdkThrownError, safeDiagnostic, failureError, } from "./turn-failure.js";
export class SdkCancelled extends Error {
}
export class SdkDeadline extends Error {
}
const MAX_TIMER = 2_147_483_647;
export function sdkDeadline(env, name) {
    const key = `MUSE_CODE_ACP_${name}_TIMEOUT_MS`;
    const value = env[key];
    if (value === undefined)
        return name === "STARTUP" ? 120_000 : 30_000;
...
export class SdkOperation {
    ...
    since = Date.now();
    constructor(stop, log = () => { }) {
        this.stop = stop;
        this.log = log;
        void this.stopped.catch(() => { });
    }
    enter(phase, timeoutMs) {
        this.check();
        ...
        if (timeoutMs !== undefined)
            this.timer = setTimeout(() => this.fail(new SdkDeadline(`Muse SDK ${phase} timed out after ${timeoutMs}ms`)), timeoutMs);
    }
    fail = (error) => {
    ...
    error(error, env, diagnostic) {
        ...
        const recovery = phase === "forking"
            ...
            : notSubmitted
                ...
                : failure?.recovery;
        const detail = safeDiagnostic(enrichment ? base.message : cause instanceof Error ? cause.message : base.message, env);
