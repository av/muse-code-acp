...
export const EARLY_CONTINUE_TEXT = "Continue from the exact point you stopped. The previous model stop was not a finished answer. Use any tool result you have not used yet, then write the rest of the reply. Do not stop immediately after a tool call.";
...
export function finishReasonCutOff(finishReason) {
 ...
 */
export function turnStoppedEarly(items, turnId, finishReason) {
    if (finishReasonCutOff(finishReason))
        return true;
    const mine = items.filter((item) => item.turnId === turnId);
    let lastTool = -1;
    let agentAfterTool = false;
    mine.forEach((item, index) => {
        if (TOOL_KINDS.has(String(item.kind)))
            lastTool = index;
