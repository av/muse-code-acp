...
const INSTALL_HINT = "Install Muse Code (the `muse` CLI) and make sure it is on PATH, " +
 ...
 */
export function museCliPath(env = process.env) {
    const override = env.MUSE_CODE_EXECUTABLE;
    if (override) {
        if (!isExecutableFile(override)) {
            throw new Error(`MUSE_CODE_EXECUTABLE points to "${override}", which is not an executable file. ${INSTALL_HINT}`);
        }
