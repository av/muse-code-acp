...
export async function withSdkControlHost(options, read) {
    const startupMs = sdkDeadline(options.env, "STARTUP");
    if (options.signal?.aborted)
        throw new SdkCancelled("Muse SDK read cancelled");
    ...
    try {
        ...
        const host = await operation.wait(handshake.initialize({ clientInfo: { name: "muse_code_acp", version: packageJson.version } }));
