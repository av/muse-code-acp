...
export function museMcpServers(mcpServers) {
    return Object.fromEntries(mcpServers.map((server) => {
        ...
        try {
            ...
            throw RequestError.invalidParams(undefined, "invalid HTTP MCP URL or headers; use http/https without URL credentials and valid header fields");
        }
        return [
            server.name,
            {
...
function existingMcpServers(settings) {
    ...
    return Object.fromEntries(Object.entries(combined).map(([name, value]) => {
        if (!isRecord(value) || value.transport === undefined)
            return [name, value];
...
export function readConfiguredMcpServers(baseEnv = process.env) {
 ...
 */
export function createMuseMcpOverlay(mcpServers, baseEnv = process.env, executionConfig, provider) {
    ...
    try {
        ...
        if (executionConfig) {
            ...
            for (const [name, server] of Object.entries(injected)) {
                injected[name] = { ...server, mode: "required" };
            }
        }
        else {
            for (const [name, value] of Object.entries(injected)) {
                ...
                if (type !== "stdio")
                    throw RequestError.invalidParams(undefined, "legacy exec supports only stdio MCP servers");
...
function readSettingsDocument(path) {
    let text;
    try {
...
function mirrorDirectory(source, destination, excludedName) {
    let names;
    try {
