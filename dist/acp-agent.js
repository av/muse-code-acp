import { OUTPUT_EXTENSION, OUTPUT_METHOD, MAX_OUTPUT_READ, supportsStoredOutput, parseOutputRequest, restoredOutputUpdates, } from "./stored-output.js";
import { COMPAT_STEER_METHOD, supportsCompatibleSteering, parseCompatibleSteering, } from "./steering-protocol.js";
...
import { observedFailure } from "./turn-failure.js";
import { SessionProgress, USAGE_EXTENSION } from "./session-progress.js";
...
import { PROVIDER_EXTENSION, RECOMMENDATION_EXTENSION, parseClientProvider, providerBinding, } from "./client-provider.js";
import { randomUUID } from "node:crypto";
import { isAbsolute } from "node:path";
import { realpathSync, statSync } from "node:fs";
import packageJson from "../package.json" with { type: "json" };
import { credentialsConfigured, credentialStatus, AUTH_EXTENSION, META_API_KEY_METHOD_ID, MUSE_LOGIN_METHOD_ID, museAuthMethods, runMuseLogout, } from "./auth.js";
import { configRecommendations } from "./config-recommendations.js";
import { applyConfigSelection, buildConfigOptions, defaultSessionConfig, resolvedModel, } from "./config-options.js";
import { supportsFileReport, fileReportRequest, FILE_REPORT_CAPABILITIES, } from "./file-change-evidence.js";
import { forkMuseSession, FORK_METADATA } from "./session-fork.js";
import { DEFAULT_SAFETY, assertSafetyGuard, isSafetyConfig, selectSafety, safetyConfigOptions, } from "./safety-settings.js";
...
import { guardContext, modeAvailability, MODES, modeState } from "./modes.js";
import { spawnMuseExec } from "./muse-exec.js";
import { spawnMuseSdkTurn, readMuseSdkSession, MuseSdkHost } from "./muse-sdk.js";
import { readSessionPreferences, writeSessionPreferences, writeSessionEffort, writeSessionMode, } from "./session-preferences.js";
...
import { parseSteeringRequest, STEER_METHOD, STEERING_CAPABILITY, supportsSteering, } from "./steering-protocol.js";
...
function resolveWorkspace(cwd, stored = false) {
    if (!isAbsolute(cwd))
    ...
    try {
        ...
        throw RequestError.invalidParams(undefined, stored
...
export class MuseAcpAgent {
    ...
    clientCapabilities = {};
    constructor(client, logger = console, options = {}) {
        this.client = client;
        this.logger = logger;
        this.options = options;
        this.client = {
            sessionUpdate: (n) => client.sessionUpdate(applyTitleUpdate(n, this.options.env ?? process.env)),
    ...
    async prepareProvider(sessionId, meta) {
        ...
        if (value === undefined) {
            if (saved && !existing)
                throw RequestError.invalidParams(undefined, "This session requires its explicit muse/provider endpoint and credentials again; no default-provider fallback was attempted");
            return;
        }
        if (this.backend !== "sdk" || this.clientCapabilities._meta?.[PROVIDER_EXTENSION] !== 1)
            throw RequestError.invalidParams(undefined, "muse/provider must be negotiated for the SDK backend");
        const provider = parseClientProvider(value);
        if (saved && saved !== providerBinding(provider))
            throw RequestError.invalidParams(undefined, "Saved session belongs to another provider endpoint; start a new session to change endpoints");
        if (existing && JSON.stringify(existing.provider) === JSON.stringify(provider))
            return;
        const state = this.sessions.get(sessionId);
        if (state?.turnFinished || state?.sdkHost?.owner.hasActiveTurn)
            throw RequestError.invalidRequest(undefined, "Wait for the active turn before replacing provider credentials");
    ...
    authStatus(sessionId) {
        ...
        return {
            ...credentialStatus(this.options.env ?? process.env, !!sessionId && this.providers.has(sessionId)),
    ...
    async publishAuth(sessionId, session) {
        if (this.clientCapabilities._meta?.[AUTH_EXTENSION] === 1 &&
            !this.disposed &&
    ...
    progressFor(sessionId, session) {
        return (session.progress ??= new SessionProgress(sessionId, this.clientCapabilities._meta?.[USAGE_EXTENSION] === 1, async (notification) => {
    ...
    async authenticate(params) {
        ...
        if (!credentialsConfigured(this.options.env ?? process.env)) {
            throw RequestError.authRequired(undefined, params.methodId === META_API_KEY_METHOD_ID
    ...
    async publishGoal(sessionId, session, goal) {
        if (this.backend !== "sdk" || this.disposed || this.sessions.get(sessionId) !== session)
            return;
        if (JSON.stringify(session.goal) === JSON.stringify(goal))
            return;
        session.goal = goal;
    ...
    async inspectGoal(sessionId, session) {
        if (!session.goal || session.goal.status === "unknown") {
            ...
            if (!session.cancelRequested)
                await this.publishGoal(sessionId, session, goal);
        }
    ...
    async publishCatalog(sessionId, session, identity, result, cache = true) {
        ...
        if (cache)
            this.modelDiscovery.remember(session.cwd, env, result);
        ...
        if (JSON.stringify(previous) === JSON.stringify(configOptions))
            return true;
        await this.client.sessionUpdate({
            sessionId,
            update: {
    ...
    async newSession(params) {
        ...
        return this.withSessionBinding(sessionId, async () => {
            ...
            const config = defaultSessionConfig(readMuseSettings(this.providerEnv(sessionId), this.logger));
            const modelDiscovery = this.backend === "sdk"
     ...
     */
    advertiseCommands(sessionId, cwd) {
        const task = listMuseSkills(cwd, this.options.env ?? process.env, this.options.museBinary, this.logger)
            .catch((err) => {
            ...
            const availableCommands = [
                ...builtInCommands,
                ...skillsToCommands(skills).filter((command) => !reservedNames.has(command.name.toLowerCase())),
            ];
    ...
    async listSessions(params) {
        ...
        return {
            ...page,
            sessions: page.sessions.map((info) => applySessionTitle(info, this.options.env ?? process.env)),
        };
    }
    ...
    async withSessionBinding(sessionId, bind) {
        this.assertRunning();
        if (this.bindingSessions.has(sessionId) ||
            ...
            throw RequestError.invalidRequest(undefined, "session has a prompt turn or binding operation in progress");
        }
    ...
    async loadSessionState(params) {
        this.validateMcp(params.mcpServers);
        if (this.sessions.get(params.sessionId)?.activeTurn) {
            throw RequestError.invalidRequest(undefined, "Cannot load a session while a prompt is active");
        }
        let env = this.providerEnv(params.sessionId);
        const stored = listStoredSessions(null, env, this.logger).find((session) => session.sessionId === params.sessionId);
        if (!stored) {
            throw RequestError.invalidParams(undefined, `session ${params.sessionId} not found in the muse session store`);
        }
        const cwd = resolveWorkspace(params.cwd);
        const storedCwd = resolveWorkspace(stored.cwd, true);
        if (cwd !== storedCwd)
            throw RequestError.invalidParams(undefined, `session ${params.sessionId} belongs to a different workspace`);
        ...
        if (previousSession)
            previousSession.steering = undefined;
        this.assertRunning();
        ...
        this.assertRunning();
        const config = defaultSessionConfig(readMuseSettings(env, this.logger));
        let goal = { status: "unknown", reason: "No goal state observed" };
        ...
        if (this.backend === "sdk") {
            ...
            if (preferences.modelSelection)
                Object.assign(config, preferences.modelSelection);
    ...
    async forkSession(params) {
        if (!this.supportsFork())
            throw RequestError.invalidRequest(undefined, "Session fork requires supported Muse SDK host 1.1.1+");
        this.validateMcp(params.mcpServers ?? []);
        requireSingleWorkspace(params.additionalDirectories);
        const cwd = resolveWorkspace(params.cwd);
        ...
        if (extension !== undefined) {
            if (this.clientCapabilities._meta?.[FORK_METADATA] !== 1 ||
                ...
                typeof extension.lastTurnId !== "string" ||
                !extension.lastTurnId.trim())
                throw RequestError.invalidParams(undefined, "Explicit fork boundary requires negotiated muse/fork and a lastTurnId");
            lastTurnId = extension.lastTurnId;
        }
        return this.withSessionBinding(params.sessionId, async () => {
            ...
            if (!stored || resolveWorkspace(stored.cwd, true) !== cwd)
                throw RequestError.invalidParams(undefined, "Fork source not found in the requested workspace");
            await this.prepareProvider(params.sessionId, params._meta);
            env = this.providerEnv(params.sessionId);
            ...
            let result;
            try {
                result = await this.trackRead(forkMuseSession({
                    signal: this.discoveryAbort.signal,
                    sessionId: params.sessionId,
                    cwd,
                    lastTurnId,
            ...
            if (this.sessions.has(sessionId))
                throw new Error("Muse fork identity is already bound");
            ...
            writeSessionPreferences(sessionId, { reasoningEffort: config.reasoningEffort, modeId: "default" }, env);
            this.sessions.set(sessionId, {
                cwd,
                museSessionId: sessionId,
                activeTurn: null,
    ...
    async resumeSessionState(params) {
        this.validateMcp(params.mcpServers ?? []);
        if (!isAbsolute(params.cwd)) {
            throw RequestError.invalidParams(undefined, `cwd must be an absolute path, got "${params.cwd}"`);
        }
        requireSingleWorkspace(params.additionalDirectories);
        const existing = this.sessions.get(params.sessionId);
        if (existing?.turnFinished) {
            throw RequestError.invalidRequest(undefined, `session ${params.sessionId} already has a prompt turn in flight`);
        }
        ...
        const stored = existing
            ? { cwd: existing.cwd }
            : listStoredSessions(null, this.options.env ?? process.env, this.logger).find((session) => session.sessionId === params.sessionId);
        if (!stored) {
            throw RequestError.invalidParams(undefined, `session ${params.sessionId} not found in the muse session store`);
        }
        const storedCwd = resolveWorkspace(stored.cwd, true);
        ...
        if (requestedCwd !== storedCwd) {
            throw RequestError.invalidParams(undefined, `session ${params.sessionId} belongs to ${stored.cwd}; ` +
        ...
        if (existing) {
            if (params._meta?.[PROVIDER_EXTENSION] !== undefined)
                existing.modelDiscovery = this.modelDiscovery.peek(storedCwd, this.providerEnv(params.sessionId));
        ...
        const config = defaultSessionConfig(readMuseSettings(this.options.env ?? process.env, this.logger));
        let goal = { status: "unknown", reason: "No goal state observed" };
        let savedMode = "default";
        let info;
        let progress = {};
    ...
    async setSessionConfigOption(params) {
        ...
        if (this.backend === "sdk" &&
            ...
            requireAvailable(unavailable("busy", "Wait for the active turn or configuration change before changing settings"));
        ...
        if (this.backend === "sdk" && isSafetyConfig(params.configId)) {
            if (session.safetyChanging || session.turnFinished || session.sdkHost?.owner.hasActiveTurn)
                throw RequestError.invalidRequest(undefined, "Wait for the active turn before changing safety settings");
            const safety = selectSafety(session.config.safety, params.configId, params.value, this.safetyGuard());
        ...
        const config = applyConfigSelection(session.config, params.configId, params.value, this.backend === "sdk" ? session.modelDiscovery : undefined);
        if (this.backend === "sdk" && params.configId === "model") {
            const provider = readMuseSettings(this.providerEnv(params.sessionId), this.logger).provider ?? "meta";
            if (JSON.stringify(resolvedModel(config, session.modelDiscovery, provider)) ===
        ...
        if (boundProvider && config.providerId && config.providerId !== boundProvider.providerId)
            throw RequestError.invalidParams(undefined, "The selected model belongs to another provider; choose a model for this session's configured gateway");
    ...
    async setSessionMode(params) {
        ...
        if ((this.backend === "sdk" || params.modeId === "plan" || session.modeId === "plan") &&
            (session.safetyChanging || session.turnFinished || session.sdkHost?.owner.hasActiveTurn))
            throw RequestError.invalidRequest(undefined, "Wait for the active turn before changing safety or planning mode");
        if (session.modeId !== params.modeId)
            await this.changeMode(params.sessionId, session, params.modeId);
        return {};
    }
    assertWorkflowTools(session) {
        if (session.mcpServers.length ||
            Object.keys(readConfiguredMcpServers(this.options.env ?? process.env)).length)
            throw RequestError.invalidParams(undefined, "Planning and review require a session without MCP servers; read-only workspace flags do not constrain external tool effects");
    }
    async changeMode(sessionId, session, mode) {
        if (session.safetyChanging)
            throw RequestError.invalidRequest(undefined, "A safety setting change is already in progress");
        session.safetyChanging = true;
        try {
            if (mode === "plan")
    ...
    async compatibleSteer(params) {
        ...
        if (!expectedTurnId)
            throw RequestError.invalidRequest(undefined, "No active turn; compatible steering never starts a new turn");
        await this.steer({ ...params, expectedTurnId });
        return { outcome: "injected" };
    }
    async steer(params) {
        if (this.backend !== "sdk" || !supportsSteering(this.clientCapabilities))
            throw RequestError.invalidRequest(undefined, "steering was not negotiated for the SDK backend");
        const session = this.requireSession(params.sessionId);
        ...
        session.steering ??= new SteeringQueue({
            isCurrent: (target, id) => !this.disposed &&
    ...
    async prompt(params) {
        ...
        if (workflow && session.sdkHost?.owner.hasActiveTurn)
            throw RequestError.invalidRequest(undefined, "Wait for native work before starting a planning or review command");
        ...
        if (command?.local) {
            ...
            try {
                ...
                if (command.local.kind === "rename") {
                    const title = renameSession(params.sessionId, command.local.argument, this.options.env ?? process.env);
                    await this.client.sessionUpdate({
                        sessionId: params.sessionId,
                        update: {
                            sessionUpdate: "session_info_update",
                    ...
                    try {
                        ...
                        const result = await this.trackRead(connection
                            ? readModelCatalog(connection, abort.signal)
                            : discovery.discover(session.cwd));
                        const published = await this.publishCatalog(params.sessionId, session, identity, result, !connection);
                        if (session.cancelRequested)
                    ...
                    const skills = await listMuseSkills(session.cwd, this.providerEnv(params.sessionId), this.options.museBinary, this.logger);
                    this.assertRunning();
        ...
        const publishReview = async (status) => {
            if (!reviewId ||
        ...
        try {
            ...
            if (command?.notice)
                await say(command.notice);
            if (command?.status) {
                const status = command.status === "status"
            ...
            if (workflow?.kind === "plan") {
                if (session.sdkHost?.owner.hasActiveTurn)
                    throw RequestError.invalidRequest(undefined, "Wait for native work before entering planning mode");
                await this.changeMode(params.sessionId, session, "plan");
                if (command?.barePlan) {
                    await say("Plan mode enabled. Send a task to begin planning; implementation requires an explicit mode change.");
            ...
            if (session.cancelRequested || this.disposed)
                return { stopReason: "cancelled" };
            const readOnly = session.modeId === "readOnly" || session.modeId === "plan" || workflow?.kind === "review";
            ...
            if (this.backend === "sdk") {
                if (this.options.provider === "echo") {
                    throw RequestError.invalidParams(undefined, "The SDK backend requires a configured Muse provider; use the exec backend for echo");
                }
                const { providerId, profileId: discoveredProfileId } = resolvedModel(session.config, session.modelDiscovery, readMuseSettings(baseEnv, this.logger).provider ?? "meta");
                const profileId = discoveredProfileId ?? null;
                const boundProvider = this.providers.get(params.sessionId)?.provider;
                if (boundProvider && providerId !== boundProvider.providerId)
                    throw RequestError.invalidParams(undefined, "Saved model provider does not match the explicitly configured gateway; no fallback was attempted");
                ...
                const identity = sdkHostConfiguration(session.cwd, session.config, `${session.modeId}:${readOnly}`, session.mcpServers, baseEnv, this.options.museBinary);
                if (session.sdkHost &&
                ...
                if (session.cancelRequested || this.disposed)
                    return { stopReason: "cancelled" };
                if (session.sdkHost?.owner.hasActiveTurn)
                    throw RequestError.invalidRequest(undefined, "Muse is executing a host-owned turn; retry after it finishes or close the session to stop the host");
                if (!session.sdkHost) {
                    const overlay = createMuseMcpOverlay(session.mcpServers, baseEnv, session.config);
                    if ((workflow || session.modeId === "plan") &&
                        Object.keys(readConfiguredMcpServers(overlay.env)).length) {
                        overlay.cleanup();
                        throw RequestError.invalidParams(undefined, "MCP configuration changed while preparing the planning or review host");
                    }
                    ...
                    const owner = new MuseSdkHost({
                        ...
                        onCatalogConnection: (connection) => {
                            void this.trackRead(readModelCatalog(connection).then(async (result) => {
                                if (session.sdkHost?.owner === owner && !owner.closed)
                                    await this.publishCatalog(params.sessionId, session, catalogIdentity, result, false);
                        ...
                        ...(this.clientCapabilities._meta?.[SESSION_STATE_EXTENSION] === 1
                            ? {
                                onSessionState: async (state) => {
                                    if (this.disposed || this.sessions.get(params.sessionId) !== session)
                                        return;
                                    await this.client.sessionUpdate({
                                        sessionId: params.sessionId,
                                        update: {
                ...
                try {
                    ...
                    if (response.stopReason === "end_turn" && !session.cancelRequested)
                        writeSessionPreferences(params.sessionId, { modelSelection: { model: session.config.model, providerId, profileId } }, this.options.env ?? process.env);
                    await publishReview(session.cancelRequested || response.stopReason === "cancelled"
                        ? "cancelled"
                        : "completed");
            ...
            if (translator.approvalWait !== null) {
                throw RequestError.internalError(undefined, `muse requested approval for ${translator.approvalWait.toolName}, but muse 0.2.1 cannot route headless approvals through ACP; select bypassApprovals or readOnly before prompting`);
            }
            ...
            switch (outcome.kind) {
                case "completed":
                    return { stopReason: "end_turn" };
                case "usage-error":
                    throw RequestError.internalError(undefined, `muse exec rejected the invocation (exit ${outcome.code}) — ` +
    ...
    turnFailure(code, terminal) {
        ...
        if (/auth|credential|api.?key|unauthorized|log.?in|401/i.test(detail)) {
            return RequestError.authRequired(undefined, `muse provider authentication failed: ${detail}. ` +
                `Run \`muse login\` or set META_API_KEY.`);
        }
        return RequestError.internalError(undefined, detail ? `muse exec failed (exit ${code}): ${detail}` : `muse exec failed (exit ${code})`);
    }
    async readOutput(params) {
        this.assertRunning();
        if (this.backend !== "sdk" || this.clientCapabilities._meta?.[OUTPUT_EXTENSION] !== 1)
            throw RequestError.invalidRequest(undefined, "muse/output must be negotiated on the SDK backend");
        const session = this.requireSession(params.sessionId);
        const saved = await this.readSavedSession({
            sessionId: params.sessionId,
            cwd: session.cwd,
            ...
            throw RequestError.invalidRequest(undefined, "Stored output is unavailable from the public Muse host; no turn was replayed");
        });
        this.assertRunning();
        if (this.sessions.get(params.sessionId) !== session)
    ...
    async controlTask({ sessionId, target }) {
        this.assertRunning();
        ...
        if (!owner)
            throw RequestError.invalidRequest(undefined, "Task host is no longer available");
    ...
    assertRunning() {
        if (this.disposed)
            throw RequestError.invalidRequest(undefined, "agent is shutting down");
    }
    ...
    dispose() {
        ...
        this.disposal = Promise.all([
            ...
            for (const binding of this.providers.values())
                binding.overlay.cleanup();
 ...
 */
export function createAgentConnection(target, logger = console, options = {}) {
    ...
    const connection = acpAgent({ name: "muse-code-acp" })
        ...
        .onRequest(COMPAT_STEER_METHOD, parseCompatibleSteering, (ctx) => agent.compatibleSteer(ctx.params))
